        ...
        try:
            ...
            for i, tag in enumerate(heading_tags):
                ...
                if text and len(text) > 0:
                    ...
                    if any(word==text for word in search_words) and not "von Karl Marx" in text:
                        continue
                    if i==0:
                        title=text                                      
                    # 确定标题级别
...
\3""",recontent,flags=re.DOTALL|re.IGNORECASE)
        ...
        content = recontent.replace(r"&szlig;", r"ß")
        ...
        for chapter in volume.chapters:
            ...
            if chapter['headings']:
                for heading in chapter['headings']:
                    text = heading['text']
                    text=re.sub(r"Karl Marx\s+/\s+Friedrich Engels",r"Karl Marx/Friedrich Engels",text,flags=re.DOTALL|re.IGNORECASE)
                    text=re.sub(r"([\[]*Karl Marx[\s\]]*|[\[]*Friedrich Engels[\s\]]*|[\[]*Karl Marx/Friedrich Engels[\s\]]*)[\s]+",r"\1 – ",text,flags=re.DOTALL|re.IGNORECASE)
                    level = heading['level']
                    headid = heading['id']
                    tag=heading['tag']
...
<span style="font-size:1.2em;"><i>Fußnoten</i></span><BR><BR>''', fixed_content, flags=re.DOTALL | re.IGNORECASE)
                ...
                fixed_content=re.sub(r'<a></a>',r'',fixed_content,flags=re.IGNORECASE|re.DOTALL)
                #if k==42:
...
def main():
    ...
    book_dir = r"./23-251"  # 修改为你的书籍根目录
    #output_dir = "./MEA-DE"  # 输出目录名
    output_dirs = [r"../mlread/docs/MEW-ZENO",r"../MARX-ZH-CN-node/docs/MEW-ZENO"] 
