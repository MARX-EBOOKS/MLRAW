...
class PageBarManager {
  ...
  _setupInteractionHandlers() {
    ...
    this._contentClickHandler = (e) => {
      ...
      this._revealPageMarker({ rect: this._pointRect(e.clientY), strategy: 'nearby', source: 'content-click', force: true, toggle: true });
    };
  ...
  _revealPageMarker(options = {}) {
    const info = options.pageInfo
      ...
      : this._findPageForRect(options.rect, options.strategy || 'current');
  ...
  _queueScrollPageMarker() {
    ...
    this._scrollMarkerRaf = requestAnimationFrame(() => {
      this._scrollMarkerRaf = null;
      this._revealPageMarker({ rect: this._pointRect(Math.max(80, innerHeight * 0.42)), strategy: 'current', source: 'scroll' });
    });
  }
  ...
  _showMarkerNearSelection() {
    ...
    return this._revealPageMarker({ rect: { top, bottom }, strategy: 'nearby', source: 'content-selection', force: true });
  }
  ...
  _findPageForRect(rect, strategy = 'current') {
    ...
    for (const pageInfo of this.pageNumbers) {
      const anchorRect = pageInfo.el.getClientRects()[0] || pageInfo.el.getBoundingClientRect();
      if (!Number.isFinite(anchorRect.top)) continue;
      const tolerance = Math.max(96, this._lineHeightFor(pageInfo.el) * 4);
      const distance = mid < anchorRect.top
        ? anchorRect.top - mid
        : mid > anchorRect.bottom
          ? mid - anchorRect.bottom
          : 0;
  ...
  _lineHeightFor(el) {
    ...
    return parseFloat(own.lineHeight) || parseFloat(parent?.lineHeight) || 32;
  }
  ...
  _showPageMarker(pageInfo, options = {}) {
    ...
    const lineHeight = this._lineHeightFor(anchor);
